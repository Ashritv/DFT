...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller (
  ...
  output wire [5:0] BIST_COL_ADD, // column address
  output wire [5:0] BIST_ROW_ADD, // row address
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_add_format (
  input  wire [5:0] Y_ADDRESS,
  input  wire [5:0] X_ADDRESS,
  output wire [5:0] COLUMN_ADDRESS,
  output wire [5:0] ROW_ADDRESS
);
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_add_gen (
  ...
  output wire [5:0] X_ADDRESS,
  output wire [5:0] Y_ADDRESS,
  output wire A_EQUALS_B_TRIGGER
);
    wire    [5:0]    AX_ADD_WIRE, BX_ADD_WIRE;
    reg     [5:0]    AX_ADD_REG;
    reg     [5:0]    BX_ADD_REG;
    wire    [5:0]    AY_ADD_WIRE, BY_ADD_WIRE;
    reg     [5:0]    AY_ADD_REG;
    reg     [5:0]    BY_ADD_REG;
    wire    [5:0]    AX_MASK;   
    wire    [5:0]    BX_MASK;
    ...
    wire   [5:0]     Y_R, Y_RP, Y_RCO, Y_RCI;
    reg    [5:0]     NEXT_Y_ADD_COUNT;
    wire   [5:0]     Y_ADD_CNT;
    wire   [5:0]     Y_ADD_CNT_TO_BUF;
    wire   [5:0]     Y_ADD_CNT_MAX, Y_ADD_CNT_MIN;
    wire   [2:0]     Y_ADD_ROT_OUT;
    wire   [1:0]     Y_ADD_ROT_IN_REG;
    wire   [5:0]     X_R, X_RP, X_RCO, X_RCI;
    reg   [5:0]      NEXT_X_ADD_COUNT;
    wire   [5:0]     X_ADD_CNT_TO_BUF;
    wire   [5:0]     X_ADD_CNT;
    wire   [5:0]     X_ADD_CNT_MAX, X_ADD_CNT_MIN;
    ...
    wire   [5:0]     X_ADD_REG_MIN_DEFAULT;
    wire   [5:0]     X_ADD_REG_MAX_DEFAULT;
    ...
    wire   [5:0]     Y_ADD_REG_MIN_DEFAULT;
    wire   [5:0]     Y_ADD_REG_MAX_DEFAULT;
...
//------------------------------
    ...
    assign Y_RP      = (({6{INCY1}} & (~Y_R)) | ({6{DECY1}} & Y_R));
 
    assign Y_RCO     = (Y_RCI | Y_RP) ;
 
    assign Y_RCI     = (~Y1_CNT_EN) ? {6{1'b1}}                                                                :
                                      ({Y_RCO[4:0], 1'b0 })                                     ;
...
//------------------------------
    ...
    assign X_RP      = (({6{INCX1}} & (~X_R)) | ({6{DECX1}} & X_R));
 
    assign X_RCO     = (X_RCI | X_RP) ;
 
    assign X_RCI     = (~X1_CNT_EN) ? {6{1'b1}}                                                                :
                                      ({X_RCO[4:0], 1'b0 })                                     ;
...
//---------------------------------
  ...
  // Instantiate Y_ADD_CNT persistent buffers {{{
    ...
    memlibc_memory_bist_assembly_rtl_tessent_buf tessent_persistent_cell_Y_ADD_CNT_5 (
        .a           (Y_ADD_CNT_TO_BUF[5]),
        .y           (Y_ADD_CNT[5])
    );    
  // Instantiate Y_ADD_CNT persistent buffers }}}
...
//----------------------------------------------
    ...
    assign X_ADD_REG_MIN_DEFAULT    = 6'b000000;
      
    assign X_ADD_CNT_MIN            = X_ADD_REG_MIN_DEFAULT;
 
    // Algorithm: SMARCH Type: IC
    assign X_ADD_REG_MAX_DEFAULT    = MBISTPG_REDUCED_ADDR_CNT_EN ? 6'b111111 : 6'b111111;
 
    assign X_ADD_CNT_MAX            = X_ADD_REG_MAX_DEFAULT;
//----------------------------------------
// Select out bit X register            --
//----------------------------------------
    assign X_ADD_ROT_OUT_DEFAULT    = 3'b101;
...
//-----------------------------------------------------------------
    ...
    case (X_ADD_ROT_IN_SELECTED) 
     2'b00 : begin
                     AX_ROT_IN_SELECTED =     AY_ADD_REG[5] ;
     end
     2'b01 : begin
    ...
    case (X_ADD_ROT_IN_SELECTED) 
     2'b00 : begin
                     BX_ROT_IN_SELECTED =     BY_ADD_REG[5] ;
           end
     2'b01 : begin
...
//----------------------------------------------
    // Algorithm: SMARCH Type: IC
    assign Y_ADD_REG_MIN_DEFAULT    = 6'b000000;
 
    assign Y_ADD_CNT_MIN            = Y_ADD_REG_MIN_DEFAULT;
 
    // Algorithm: SMARCH Type: IC
    assign Y_ADD_REG_MAX_DEFAULT    = MBISTPG_REDUCED_ADDR_CNT_EN ? 6'b111111 : 6'b111111;
...
//----------------------------------------
    assign Y_ADD_ROT_OUT_DEFAULT    = 3'b101;
...
//------------------------------------------------
    ...
    case (Y_ADD_ROT_OUT)
     ...
     3'b101 : begin
                     AY_ROT_OUT_SELECTED = AY_ADD_REG[5:5];
                     BY_ROT_OUT_SELECTED = BY_ADD_REG[5:5];
              end
    default : begin 
...
//------------------------------------
    assign           AX_ADD_WIRE    = 6'b000000; // Algorithm: SMARCH
 
    assign AX_MASK   =                      (BIST_SHIFT_SHORT)    ? 6'b111111 :
                      (X_ADD_ROT_OUT == 3'b101)    ? 6'b111111 :
                      (X_ADD_ROT_OUT == 3'b100)    ? 6'b011111 :
                      (X_ADD_ROT_OUT == 3'b011)    ? 6'b001111 :
                      (X_ADD_ROT_OUT == 3'b010)    ? 6'b000111 :
                      (X_ADD_ROT_OUT == 3'b001)    ? 6'b000011 :
                                                                    6'b000001; 
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        ...
        if (BIST_SHIFT_SHORT | (LAST_TICK & (~BIST_HOLD) & ROT_LF_A_ADD_REG & BIST_RUN))
            AX_ADD_REG              <= (AX_MASK & {AX_ADD_REG[4:0], AX_ROT_IN_SELECTED}) | ((~AX_MASK) & AX_ADD_REG);
        else
        if (LAST_TICK & (~BIST_HOLD) & ROT_RT_A_ADD_REG & BIST_RUN)
            AX_ADD_REG              <= {AX_ROT_RT_IN_SELECTED, AX_ADD_REG[5:1]};
        else begin
...
//------------------------------------
    assign           AY_ADD_WIRE    = 6'b000000; // Algorithm: SMARCH
 
    assign AY_MASK   =                      (BIST_SHIFT_SHORT)    ? 6'b111111 :
                      (Y_ADD_ROT_OUT == 3'b101)    ? 6'b111111 :
                      (Y_ADD_ROT_OUT == 3'b100)    ? 6'b011111 :
                      (Y_ADD_ROT_OUT == 3'b011)    ? 6'b001111 :
                      (Y_ADD_ROT_OUT == 3'b010)    ? 6'b000111 :
                      (Y_ADD_ROT_OUT == 3'b001)    ? 6'b000011 :
                                                                    6'b000001;
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        ...
        if (BIST_SHIFT_SHORT | (LAST_TICK & (~BIST_HOLD) & ROT_LF_A_ADD_REG & BIST_RUN))
            AY_ADD_REG              <= (AY_MASK & {AY_ADD_REG[4:0], AY_ROT_IN_SELECTED}) | ((~AY_MASK) & AY_ADD_REG);
        else
        if (LAST_TICK & (~BIST_HOLD) & ROT_RT_A_ADD_REG & BIST_RUN)
            AY_ADD_REG              <= {AY_ROT_RT_IN_SELECTED, AY_ADD_REG[5:1]};
        else begin
          if ( ENABLE_A_ADD_REG & BIST_RUN & (~BIST_HOLD))
              if (LAST_TICK & (~INH_LAST_ADDR_CNT)) begin
...
//------------------------------------
    assign           BX_ADD_WIRE    = 6'b000000; // Algorithm: SMARCH
 
    assign BX_MASK   =                      (BIST_SHIFT_SHORT)    ? 6'b111111 :
                      (X_ADD_ROT_OUT == 3'b101)    ? 6'b111111 :
                      (X_ADD_ROT_OUT == 3'b100)    ? 6'b011111 :
                      (X_ADD_ROT_OUT == 3'b011)    ? 6'b001111 :
                      (X_ADD_ROT_OUT == 3'b010)    ? 6'b000111 :
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        ...
        if (BIST_SHIFT_SHORT | (LAST_TICK & (~BIST_HOLD) & ROT_LF_B_ADD_REG & BIST_RUN))
            BX_ADD_REG              <= (BX_MASK & {BX_ADD_REG[4:0], BX_ROT_IN_SELECTED}) | ((~BX_MASK) & BX_ADD_REG);
        else begin
...
//------------------------------------
    assign           BY_ADD_WIRE    = 6'b000000; // Algorithm: SMARCH
 
    assign BY_MASK   =                      (BIST_SHIFT_SHORT)    ? 6'b111111 :
                      (Y_ADD_ROT_OUT == 3'b101)    ? 6'b111111 :
                      (Y_ADD_ROT_OUT == 3'b100)    ? 6'b011111 :
                      (Y_ADD_ROT_OUT == 3'b011)    ? 6'b001111 :
                      (Y_ADD_ROT_OUT == 3'b010)    ? 6'b000111 :
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        ...
        if (BIST_SHIFT_SHORT | (LAST_TICK & (~BIST_HOLD) & ROT_LF_B_ADD_REG & BIST_RUN))
            BY_ADD_REG              <= (BY_MASK & {BY_ADD_REG[4:0], BY_ROT_IN_SELECTED}) | ((~BY_MASK) & BY_ADD_REG);
        else begin
          if ( ENABLE_B_ADD_REG & BIST_RUN & (~BIST_HOLD))
              if (LAST_TICK & (~INH_LAST_ADDR_CNT)) begin
...
//------------------------------------------------
    ...
    assign A_SCAN_REGISTER_SI       = AX_ADD_REG[5];
...
//------------------------------------------------
    ...
    assign B_SCAN_REGISTER_SI       = BX_ADD_REG[5];
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_data_gen (
  ...
  input wire [3:0] WDATA_CMD,
  input wire [3:0] EDATA_CMD,
  input wire [5:0] ROW_ADDRESS,
  input wire [5:0] COLUMN_ADDRESS,
  input wire LAST_TICK,
  input wire BIST_RUN,
  input wire BIST_SHIFT_SHORT,
  input wire SI,
  ...
  output wire SO
    ...
    wire             EDATA_SI;
    wire             WDATA_SI;
    wire   [5:0]     X_ADDR_BIT_SEL_DECODED;
    wire   [5:0]     Y_ADDR_BIT_SEL_DECODED;
...
//----------------------------
    ...
    assign Y_ADDR_BIT_SEL_DECODED[5]              = 1'b0;
